...
import { FT, workoutColor } from '../_components/tokens'
...
type WeekGroup = {
  label: string
  monday: string
  workouts: Awaited<ReturnType<typeof getAllWorkouts>>
  totalVolume: number
}

function groupByWeek(workouts: Awaited<ReturnType<typeof getAllWorkouts>>): WeekGroup[] {
  const map = new Map<string, WeekGroup>()
  for (const w of workouts) {
    const monday = getWeekMonday(w.date)
    let g = map.get(monday)
    if (!g) {
      g = { label: getWeekKey(w.date), monday, workouts: [], totalVolume: 0 }
      map.set(monday, g)
    }
    g.workouts.push(w)
...
export default async function TrainPage({
  ...
  const [allWorkouts, calendarWorkouts, exerciseNames] = await Promise.all([
    getAllWorkouts(60),
    getWorkoutsForCalendar(calYear, calMonth),
    getDistinctExerciseNames(),
  ])
  ...
  if (selectedExercise) {
    ;[exerciseProgression, exerciseSessions] = await Promise.all([
      ...
      getExerciseSessionHistory(selectedExercise),
    ])
  }

  const calByDate = new Map(calendarWorkouts.map((w) => [w.date, w]))

  type WorkoutWithLogs = { workout: Awaited<ReturnType<typeof getAllWorkoutsForDate>>[0]; logs: Awaited<ReturnType<typeof getExerciseLogsForWorkout>> }
  let selectedWorkouts: WorkoutWithLogs[] = []
  if (selectedDay) {
    const dayWorkouts = await getAllWorkoutsForDate(selectedDay)
    selectedWorkouts = await Promise.all(
      dayWorkouts.map(async (w) => ({
        workout: w,
        logs: await getExerciseLogsForWorkout(w.id),
      }))
    )
  }
  ...
  const monthLabel = new Date(calYear, calMonth - 1, 1).toLocaleDateString('en-IN', {
    month: 'long',
    year: 'numeric',
  })
    ...
    <div className="space-y-5">
      ...
      <div className="rounded-xl p-3" style={{ background: FT.surface, border: `1px solid ${FT.border}` }}>
        <div className="flex items-center justify-between mb-3">
          <Link
            href={`/fittrack/train?year=${prevY}&month=${prevM}`}
          ...
          <Link
            href={`/fittrack/train?year=${nextY}&month=${nextM}`}
            className="p-1 rounded"
            style={{ color: FT.textMuted }}
          >
            →
        ...
        <div className="grid grid-cols-7 gap-0.5">
          {cells.map((day, i) => {
            ...
            return (
              <Link
                key={iso}
                href={`/fittrack/train?day=${iso}&year=${calYear}&month=${calMonth}`}
              ...
              >
                ...
                {wc && (
                  <span
                    ...
                    style={{ width: 5, height: 5, background: wc.text }}
                  />
                )}
              </Link>
            )
          ...
          {selectedWorkouts.length > 0 ? (
            <div className="space-y-2">
              {selectedWorkouts.map(({ workout, logs }) => (
                <WorkoutCard key={workout.id} workout={workout} exerciseLogs={logs} linkableExercises editable />
              ))}
            </div>
          ) : (
            <div
              className="rounded-xl p-4 text-center text-sm"
              style={{ background: FT.surface, border: `1px solid ${FT.border}`, color: FT.textMuted }}
            >
              Rest day
            </div>
          )}
              ...
              <div className="space-y-1">
                {week.workouts.map((w) => {
                  ...
                  return (
                    <Link
                      key={w.id}
                      href={`/fittrack/train?day=${w.date}&year=${calYear}&month=${calMonth}`}
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate" style={{ color: FT.textPrimary }}>
                          {w.title || w.notes || wc.label}
                        </p>
            ...
            {exerciseNames.map((name, i) => (
              <Link
                ...
                style={{
                  ...
                  borderTop: i > 0 ? `1px solid ${FT.borderSubtle}` : 'none',
                }}
              >
