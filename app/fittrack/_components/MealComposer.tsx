...
import {
  ...
  scaleFoodMacros,
  sumComponents,
} from '@/lib/fittrack/macro-compute'
...
export function MealComposer({
  ...
  const grouped = useMemo(() => {
    const m = new Map<string, FoodLibraryItem[]>()
  ...
  function addFood(food: FoodLibraryItem) {
    ...
    setPicked((prev) => {
      ...
      if (existingIdx >= 0) {
        const next = [...prev]
        const newQty = next[existingIdx].qty + qty
        const scaled = scaleFoodMacros(food, newQty)
        next[existingIdx] = {
          ...next[existingIdx],
          qty: newQty,
          ...scaled,
  ...
  async function handleLog() {
    ...
    setPending(true)
    setError(null)
    const res = await addMealWithComponentsAction({
      date,
      meal_type: mealType,
      components: picked,
    })
  ...
  return (
    ...
    >
      ...
      <div className="flex gap-1 overflow-x-auto -mx-1 px-1">
        {categories.map((c) => (
          <button
            ...
            style={{
              background: category === c ? FT.accent : FT.accentBg,
              color: category === c ? '#fff' : FT.accent,
            }}
          >
            {c}
          </button>
      ...
      <div
        ...
        style={{ border: `1px solid ${FT.borderSubtle}` }}
      >
        {grouped.length === 0 ? (
          <p className="p-3 text-xs" style={{ color: FT.textMuted }}>
            No items match.
          </p>
        ) : (
            ...
            <div key={cat}>
              ...
              {items.map((f) => (
                <button
                  ...
                  type="button"
                  onClick={() => addFood(f)}
                  className="w-full flex items-center justify-between px-2.5 py-1.5 text-left hover:bg-[color:var(--hov)] transition-colors"
                  style={{
                    ['--hov' as string]: FT.canvas,
                    borderTop: `1px solid ${FT.borderSubtle}`,
                  }}
                >
                  <div className="min-w-0 flex-1">
                    <p className="text-xs font-medium truncate" style={{ color: FT.textPrimary }}>
                      {f.name}
                    </p>
                    <p className="text-[10px]" style={{ color: FT.textMuted }}>
                      per {f.serving_size_g ?? '-'}
                      {f.serving_unit ?? 'g'}: {f.calories ?? 0}cal · {f.protein_g ?? 0}p ·{' '}
                      {f.carbs_g ?? 0}c · {f.fat_g ?? 0}f
                    </p>
                  </div>
                  <span
                    className="ml-2 rounded px-2 py-0.5 text-[10px] font-semibold flex-shrink-0"
      ...
      {picked.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[10px] uppercase tracking-wide font-semibold" style={{ color: FT.textSecondary }}>
          ...
          {picked.map((c, i) => (
            ...
            >
              <div className="min-w-0 flex-1">
                ...
                <p className="text-[10px]" style={{ color: FT.textMuted }}>
                  {c.calories}cal · {c.protein_g}p · {c.carbs_g}c · {c.fat_g}f
                </p>
              </div>
              <input
                type="number"
      ...
      <div
        ...
        style={{ background: FT.accentBg }}
      >
        <p className="text-xs font-semibold" style={{ color: FT.accent }}>
          Total
        </p>
        <p className="text-xs font-semibold" style={{ color: FT.accent }}>
          {totals.calories}cal · {totals.protein_g}p · {totals.carbs_g}c · {totals.fat_g}f
        </p>
      </div>
