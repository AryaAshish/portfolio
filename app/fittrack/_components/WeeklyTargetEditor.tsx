...
export function WeeklyTargetEditor({ current }: { current: number }) {
  ...
  return (
    ...
    >
        ...
        {CHOICES.map((n) => {
          ...
          return (
            <button
              ...
              style={{
                background: active ? FT.accent : FT.surface,
                color: active ? '#ffffff' : FT.textSecondary,
                border: `1px solid ${active ? FT.accent : FT.border}`,
      ...
      {error && (
        <p className="text-xs mt-2" style={{ color: FT.danger }}>
          {error}
        </p>
      )}
    </div>
