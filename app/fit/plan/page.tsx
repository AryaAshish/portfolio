...
import { getPlanPhases } from '@/lib/fit-public/data'
import { PhaseEditorPublic } from '../_components/PhaseEditorPublic'
...
export default async function FitPlanPage() {
  const client = createFitPublicServerClient()
  const phases = await getPlanPhases(client)
  const fitDefault = cookies().get('fit_default_landing')?.value === '1'
  ...
  return (
    <div className="max-w-2xl mx-auto space-y-4 py-4 px-3">
      ...
      <PhaseEditorPublic initial={phases} />
      ...
      <div className="pt-6 space-y-3" style={{ borderTop: `1px solid ${FT.border}` }}>
        ...
        <DefaultLandingToggle initial={fitDefault} />
        <a
          href="/api/fit/export"
          download
