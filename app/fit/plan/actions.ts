...
import { replacePlanPhases, resetPlanPhasesToDefaults } from '@/lib/fit-public/data'
...
export async function savePhasesAction(phases: unknown) {
  ...
  try {
    ...
    await replacePlanPhases(client, renumbered)
    revalidatePath('/fit')
    revalidatePath('/fit/plan')
...
export async function resetPhasesAction() {
  try {
    const client = createFitPublicServerClient()
    await resetPlanPhasesToDefaults(client)
    revalidatePath('/fit')
    revalidatePath('/fit/plan')
    return { ok: true as const }
  } catch (e) {
