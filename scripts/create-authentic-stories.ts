...
async function createAuthenticStories() {
...
Over 4 days, I completed my certification dives at Nursery, Margerita, and Bus Stop sites. Saw nudibranch, barracuda, tuna. But the real discovery wasn't the marine life - it was learning that most fears are just stories we tell ourselves.
...
**Dive Stats:**
- 4 certification dives + 1 fun dive
- Max depth: 18 meters
- Sites: Nursery, Margerita, Bus Stop
- Instructor: Ahmed, Scuba Yogi Dive Center
- Marine life spotted: Nudibranch, Barracuda, Tuna, Clownfish, Sea Anemones

**What I learned:**
1. Panic is normal. Pushing through it is what matters.
2. Good instructors don't rush you. Ahmed let me fail and try again.
3. The underwater world is worth every moment of fear.

If you're thinking about getting certified - do it. Your brain will try to stop you. Don't listen.`,
      image: null // Will be replaced from admin panel
    },
...
1. No agenda. Just going with the flow.
2. Hostel culture - shared spaces force you to interact
3. Bad weather = good bonding (stuck together = forced to talk)
4. Solo travel doesn't mean lonely travel

If you're thinking about solo travel but scared of being alone - don't be. You won't be alone for long. Hostels, bad weather, and bonfire conversations have a way of turning strangers into friends.
...
Instead, I spent 30 minutes underwater, exploring a world I'd only seen in documentaries. Saw clownfish hiding in anemones (yes, like Finding Nemo). Watched a sea turtle glide past like it owned the place. Touched coral (gently - the instructor made sure I knew not to damage it).
...
- Depth: 12 meters
- Duration: 30 minutes
- Type: Discover Scuba (trial dive)
- Cost: ₹4500
- Saw: Clownfish, sea turtle, coral reefs, yellow tangs
...
Sometimes the journey is harder than expected. That's not a bug. That's the point.`,
  ...
  console.log('Creating authentic life stories...\n')

  const { data, error } = await supabase
    .from('content_pages')
    .upsert({
      page_type: 'life',
      content: authenticStories,
  ...
  console.log('✅ Life stories created successfully!')
  console.log(`\n${authenticStories.length} authentic stories:`)
  authenticStories.forEach(story => {
    console.log(`\n📖 ${story.title}`)
    console.log(`   ${story.type.toUpperCase()} | ${story.location}`)
    console.log(`   ${story.description.split('\n\n')[0]}...`)
  })
  
  console.log('\n\n💡 These stories are designed to:')
  console.log('   1. Show transformation (fear → confidence)')
  console.log('   2. Include vulnerability (real struggles)')
  console.log('   3. Provide actionable takeaways')
  console.log('   4. Use sensory details (make readers feel present)')
  console.log('   5. Connect to universal emotions')
  console.log('\n📝 Images are set to null - replace from admin panel')
}

createAuthenticStories().catch(console.error)
